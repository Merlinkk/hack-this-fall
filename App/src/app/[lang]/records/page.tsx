...
export default function RecordsPage() {
  ...
  // Translations
  const medicalRecords = useTranslatedText('Medical Records');
  const completeHistory = useTranslatedText('Your complete health history');
  const searchRecords = useTranslatedText('Search records...');
  const download = useTranslatedText('Download');
  const viewDetails = useTranslatedText('View Details');
  const noRecordsFound = useTranslatedText('No records found');
  const tryAdjusting = useTranslatedText('Try adjusting your filters');
      ...
      <div className="p-4 space-y-6">
        ...
        {filteredRecords.length === 0 && (
          ...
          >
            ...
            <p className="text-gray-500 font-medium">{noRecordsFound}</p>
            <p className="text-sm text-gray-400 mt-1">{tryAdjusting}</p>
          </motion.div>
        )}
      </div>
