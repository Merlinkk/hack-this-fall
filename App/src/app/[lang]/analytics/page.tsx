...
// Metrics will be translated in the component
const metricKeys = [
  { key: 'Average Heart Rate', value: '74 bpm', change: '+2.3%', trend: 'up' as const, icon: Heart, color: 'text-red-500', bgColor: 'bg-red-500/10' },
  { key: 'Daily Steps', value: '9,471', change: '+12.5%', trend: 'up' as const, icon: Activity, color: 'text-blue-500', bgColor: 'bg-blue-500/10' },
  { key: 'Hydration', value: '2.0L', change: '-5.2%', trend: 'down' as const, icon: Droplet, color: 'text-cyan-500', bgColor: 'bg-cyan-500/10' },
  { key: 'Calories Burned', value: '2,340', change: '+8.1%', trend: 'up' as const, icon: Apple, color: 'text-orange-500', bgColor: 'bg-orange-500/10' },
];
...
export default function AnalyticsPage() {
  ...
  // Translations
  const healthAnalytics = useTranslatedText('Health Analytics');
  const trackMetrics = useTranslatedText('Track your health metrics and progress');
  const weeklyOverview = useTranslatedText('Weekly Overview');
  const steps = useTranslatedText('Steps');
  const heartRate = useTranslatedText('Heart Rate (bpm)');
  const hydration = useTranslatedText('Hydration (ml)');
  const insights = useTranslatedText('Insights');
  
  // Translate metric labels (must be done at component level, not in map)
  const averageHeartRate = useTranslatedText('Average Heart Rate');
  const dailySteps = useTranslatedText('Daily Steps');
  const hydrationLabel = useTranslatedText('Hydration');
  const caloriesBurned = useTranslatedText('Calories Burned');
  
  // Create translated metrics array
  const translatedMetrics = [
    { ...metricKeys[0], label: averageHeartRate },
    { ...metricKeys[1], label: dailySteps },
    { ...metricKeys[2], label: hydrationLabel },
    { ...metricKeys[3], label: caloriesBurned },
  ];
        ...
        >
          <Card className="p-6 border-2 border-border/50 rounded-2xl">
            <div className="flex items-center justify-between mb-6">
              <h2 className="text-xl font-extrabold text-gray-900">{weeklyOverview}</h2>
            ...
            <div className="mb-6">
              <div className="flex items-center justify-between mb-3">
                <p className="text-sm font-semibold text-gray-700">{heartRate}</p>
            ...
            <div>
              <div className="flex items-center justify-between mb-3">
                <p className="text-sm font-semibold text-gray-700">{hydration}</p>
