...
export default function MediScanPage() {
  ...
  const subtitle = useTranslatedText('Scan medicine packages to get instant information');
  const uploadText = useTranslatedText('Upload or capture a photo of your medicine package');
  const cameraLabel = useTranslatedText('Camera');
  const galleryLabel = useTranslatedText('Gallery');
  const scanButton = useTranslatedText('Scan Medicine');
  const scanningText = useTranslatedText('Scanning...');
  const medicineInfo = useTranslatedText('Medicine Information');
  const medicineName = useTranslatedText('Medicine Name');
  const generic = useTranslatedText('Generic');
  const expiryDate = useTranslatedText('Expiry Date');
  const purpose = useTranslatedText('Purpose');
  const dosageForm = useTranslatedText('Dosage & Form');
  const manufacturer = useTranslatedText('Manufacturer');
  const batchNumber = useTranslatedText('Batch Number');
  const warnings = useTranslatedText('Warnings');
  const disclaimer = useTranslatedText('This information is extracted from the medicine package. Always verify with a healthcare professional before use.');
  const scanFailed = useTranslatedText('Scan Failed');
  const expired = useTranslatedText('Expired');
  const expiringSoon = useTranslatedText('Expiring Soon');
  const valid = useTranslatedText('Valid');
  const unknown = useTranslatedText('Unknown');
  const notFound = useTranslatedText('Not found');

  const imageErrorText = useTranslatedText('Please select an image file');
  const sizeErrorText = useTranslatedText('Image size should be less than 5MB');
  const selectImageErrorText = useTranslatedText('Please select an image first');
  const cameraErrorText = useTranslatedText('Failed to capture photo');
  const galleryErrorText = useTranslatedText('Failed to pick image from gallery');
  const scanErrorText = useTranslatedText('Failed to scan medicine. Please try again.');
  ...
  const handleCameraCapture = async () => {
    try {
      ...
      setError(error.message || cameraErrorText);
    }
  };
  ...
  const handleGalleryPick = async () => {
    try {
      ...
      setError(error.message || galleryErrorText);
    }
  };
  ...
  const handleScan = async () => {
    ...
    try {
      ...
      const errorMsg = err.message || scanErrorText;
      setError(errorMsg);
      setScanResult({
        success: false,
        error: errorMsg
            ...
            >
              <Card className="p-6 bg-green-50 border-green-200">
                <div className="flex items-center gap-2 mb-4">
                  ...
                  <h2 className="text-xl font-bold text-gray-900">{medicineInfo}</h2>
                </div>
                ...
                <div className="space-y-4">
                  ...
                  {(scanResult.medicine.dosage || scanResult.medicine.form) && (
                    <div>
                      <h3 className="font-semibold text-gray-700 mb-1">{dosageForm}</h3>
                      <p className="text-gray-900">
                  ...
                  {scanResult.medicine.batchNumber && (
                    <div>
                      <h3 className="font-semibold text-gray-700 mb-1">{batchNumber}</h3>
            ...
            >
              <Card className="p-6 bg-red-50 border-red-200">
                <div className="flex items-center gap-2 text-red-700">
                  ...
                  <div>
                    <h3 className="font-semibold mb-1">{scanFailed}</h3>
