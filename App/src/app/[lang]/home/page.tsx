...
export default function HomePage() {
  ...
  // Sync route language with context on mount
  useEffect(() => {
    const routeLang = params?.lang as string;
    if (routeLang && SUPPORTED_LANGUAGES.some(l => l.code === routeLang)) {
      if (currentLanguage !== routeLang) {
        console.log('🔄 [HOME] Syncing route language to context:', routeLang);
        setLanguage(routeLang as LanguageCode);
      }
    }
  }, [params?.lang, currentLanguage, setLanguage]);
  
  // Translations
  const greeting = useTranslatedText('Hello');
  const user = useTranslatedText('User');
  const score = useTranslatedText('Score');
  const proMember = useTranslatedText('Pro Member');
  const searchAnything = useTranslatedText('Search anything...');
  const healthInsights = useTranslatedText('Health Insights');
  const heartRate = useTranslatedText('Heart Rate');
  const bpm = useTranslatedText('bpm');
  const stepsTaken = useTranslatedText('Steps Taken');
  const total = useTranslatedText('total');
  const hydration = useTranslatedText('Hydration');
  const ml = useTranslatedText('ml');
  const nutrition = useTranslatedText('Nutrition');
  const virtualConsultations = useTranslatedText('Virtual Consultations');
  const upcomingAppointments = useTranslatedText('Upcoming Appointments');
  const healthOverview = useTranslatedText('Health Overview');
  ...
  useEffect(() => {
    ...
    if (userInfo) {
      ...
      router.push(getLocalizedLink('/onboarding/info'));
    }
      ...
      <div className="p-4 space-y-6 bg-white">
        ...
        >
          <div className="flex items-center justify-between">
            <h2 className="text-2xl font-extrabold text-gray-900">{healthInsights}</h2>
          ...
          <div className="flex gap-3 overflow-x-auto pb-2 scrollbar-hide -mx-1 px-1">
            ...
            >
              ...
              <div className="relative z-10">
                <div className="flex items-center justify-between mb-2">
                  <span className="text-white text-xs font-bold">{heartRate}</span>
            ...
            >
              ...
              <div className="relative z-10">
                <div className="flex items-center justify-between mb-2">
                  <span className="text-white text-xs font-bold">{stepsTaken}</span>
        ...
        >
          <div className="flex items-center justify-between">
            <h2 className="text-2xl font-extrabold text-gray-900">{virtualConsultations}</h2>
            <motion.button
          ...
          <div className="relative">
            ...
            >
              ...
              <p className="text-lg font-bold text-gray-700">{upcomingAppointments}</p>
            </motion.div>
            <motion.button
              initial={{ scale: 0 }}
              animate={{ scale: 1 }}
        ...
        >
          <h2 className="text-2xl font-extrabold text-gray-900 mb-4">{healthOverview}</h2>
          <HealthMetrics />
        </motion.div>
      </div>
